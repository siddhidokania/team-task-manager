...
router.get('/:projectId', auth, async (req, res) => {
  try {
    ...
    const memberCheck = await db.get_one(
    ...
    const byStatus = await db.get_all(
      'SELECT status, COUNT(*) as count FROM tasks WHERE project_id=? GROUP BY status',
      [projectId]
    );
    const byUser = await db.get_all(
      `SELECT u.name, COUNT(t.id) as count
       FROM tasks t JOIN users u ON t.assigned_to = u.id
       WHERE t.project_id=? GROUP BY u.name`,
      [projectId]
    );
    const overdue = await db.get_all(
      `SELECT * FROM tasks WHERE project_id=? AND due_date < date('now') AND status != 'done'`,
      [projectId]
    );
